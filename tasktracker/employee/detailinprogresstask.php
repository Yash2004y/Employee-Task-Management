...
<body>
                  ...
                  <table class="table table-bordered">
                    ...
                    <?php
                      if ($conn && isset($_REQUEST["id"])) {
                        $per = 0;
                        $cmd =  "select atask.*,task.* from allocatetasks atask,tasks task where atask.empId = '$emp[empId]' and atask.percentage between '1' and '99' and atask.taskId = task.taskId and task.taskId = '$_REQUEST[id]'";
      ...
      <div class="col-8 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            ...
            <form class="forms-sample" onsubmit="return valid()" action="action/detailinprogresstask.php?id=<?php echo $_REQUEST["id"]; ?>" method="post">

              <div class="form-group">
                <label for="msg">Remark</label>
                <textarea class="form-control" id="msg" name="msg" placeholder="Remark" rows="4" autofocus></textarea>
              </div>

              <div class="form-group">
                <label for="per">Task Progress Percentage</label>
                <input type="text" class="form-control" id="per" value="<?php echo $per; ?>" onkeyup="chgstatus(this)" onkeydown="chgstatus(this)" placeholder="Progress Percentage" name="per">
              </div>
              ...
              <button type="submit" value="submit" name="submit" class="btn btn-gradient-success me-2">Update Status</button>
  ...
  <script>
    ...
    function valid() {
      ...
      previousPer = parseInt(<?php echo $per; ?>);
      if (per.value.trim() == "") {
        swal({
        ...
        return false;
      }else if(parseInt(per.value) < previousPer)
      {
        swal({
            text: "Percentage Must Be Grater Than Previous Progress " + previousPer + "%",
            icon: "error",
            button: {
              text: "OK",
              className: "btn btn-gradient-danger",
